...
import { devGuardians, potCredit } from './dev-federation.mjs'
...
// deposit `amt` (display units, div 0 here) for `who` via the CURRENT pot model: `who` sends the rune
// from its own Taproot address to the bakery pot, and the node credits the unique spender (`who`).
function deposit(who, amt) {
  const { depTxid, dep } = potCredit({ owner: who, amount: amt, net: NET, rune: RUNE, runeName: RUNE_NAME })
  return { dep, txid: depTxid }
}
