...
console.log(`   owner ${OWNER_ADDR.slice(0, 20)}…  vault ${vault.address.slice(0, 20)}…  dest ${DEST_ADDR.slice(0, 20)}…`)

// ── 2 · gas (real SPV-proven donation) + deposit (real Runestone, proven ingress) ─
...
const sendOut = JSON.parse(ord('send', '--fee-rate', '1', vault.address, `${DEPOSIT}:${RUNE_NAME}`))
mine(1); sync()
const dep = jpost('/api/kraynet/rune/deposit', { runeId: RUNE, txid: sendOut.txid, vault: _dv.params })
ok(dep.ok === true && dep.amount === DEPOSIT.toString(), `deposited ${dep.amount} ${RUNE_NAME} into the owner's vault — proven, watched`)
...
wtx.signIdx(OWNER.sk, 0)  // the vault's cooperative leaf — tapLeafScript told the wallet how
wtx.signIdx(OWNER.sk, 1)  // the funding key path — tapInternalKey told the wallet to tweak
...
ok(changeGot === DEPOSIT - EXIT, `…and the rune change (${changeGot}) is conserved on output 2 (the shared-pool pad) — nothing burned`)
...
ok(changeWatched, 'the rune change outpoint is back under the WATCHER\'s eye — the reserve never blinks')
